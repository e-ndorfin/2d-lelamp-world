...
export function detectMismatchedLLMProvider() {
  switch (EMBEDDING_DIMENSION) {
    case OPENAI_EMBEDDING_DIMENSION:
      if (!process.env.OPENAI_API_KEY && !process.env.LLM_API_URL) {
        throw new Error(
          "Are you trying to use OpenAI? If so, run: npx convex env set OPENAI_API_KEY 'your-key'\n" +
          "Or for OpenRouter/custom: npx convex env set LLM_API_URL 'your-url'",
        );
      }
      break;
...
export async function chatCompletion(
  ...
  // GPT-5 Nano compatibility: remap max_tokens, strip stop and non-default temperature.
  ...
  if (config.provider === 'openai') {
    ...
    if (requestBody.temperature !== undefined && requestBody.temperature !== 1) {
  ...
  const {
    ...
    if (body.stream) {
      ...
      if (!json.choices || json.choices.length === 0) {
        throw {
          retry: true,
          error: new Error('Empty choices in LLM response: ' + JSON.stringify(json)),
        };
      }
